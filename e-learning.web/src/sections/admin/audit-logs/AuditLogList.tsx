...
export default function AuditLogList() {
  ...
  const { tenants, schools, loaded } = useSelector((state) => state.adminMasterData);
  ...
  useEffect(() => {
    ...
    if (!userSearchInput || String(userSearchInput).trim() === '') return;
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const response = await axios.get(API_ENDPOINTS.adminAuditLogs, { params });
        ...
        console.error('AuditLogList fetch error with params:', params, error.response?.data || error.message || error);
      } finally {
  ...
  }, [page, rowsPerPage, selectedUser, filterTenant, filterSchool, filterAction, startDate, endDate]);
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await axios.get(API_ENDPOINTS.adminAuditLogsExport, { 
        params,
        responseType: 'blob' 
      });

...
CREATE TABLE curriculum_node (
    ...
    node_type   VARCHAR(100)  NOT NULL CHECK (LENGTH(BTRIM(node_type)) > 0),    -- Free-text type label (tenant-defined)
...
CREATE TABLE content_item (
    ...
    media_processing_status VARCHAR(20) NOT NULL DEFAULT 'NOT_REQUIRED',                      -- Video processing lifecycle (NOT_REQUIRED, QUEUED, PROCESSING, READY, FAILED)
    media_processing_error  TEXT,                                                              -- Last processing error message if failed
    media_processing_started_at TIMESTAMPTZ,                                                   -- Timestamp when media processing started
    media_processing_completed_at TIMESTAMPTZ,                                                 -- Timestamp when media processing completed
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_school_tenant_mapping_active ON school_tenant_mapping(tenant_id, school_id) WHERE is_deleted = FALSE;
